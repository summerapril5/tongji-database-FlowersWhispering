  ...
  <div class="user-interfacer">
    ...
    <div v-if="showDeleteConfirm" class="delete-confirm-overlay" @click="cancelDelete">
      <div class="delete-confirm-content" @click.stop>
        <h2>确认删除</h2>
        <p>您确定要删除此帖子吗？</p>
        <button @click="confirmDelete(selectedPost)">确认</button>
...
export default {
  ...
  computed: {
    ...
    // 计算总页数
    totalPagesCount() {
      return Math.ceil(this.userPosts.length / this.itemsPerPage);
    }
  ...
  methods:
  {
    ...
    confirmDelete(selectedPost) {
      console.log('post:', selectedPost.articleId); // 调试信息
      this.$store.dispatch('deletePost',  selectedPost.articleId);
      this.userPosts = this.userPosts.filter(p => p.articleId !== selectedPost.articleId);
      this.closeModal();
